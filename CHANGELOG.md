...
- **取证**（11.65→12.05M 谷期逐桶对照）：value loss 爆炸序列
  1.24→5.30→**1316**→179→0.05，与 op 谷（0.60→0.37→0.34）及 mean_ret
  巨摆（728→2388→1378）同步 —— 评论家崩→GAE 污染→策略塌。
- **根因**: ReturnNormalizer(alpha=0.01) EMA 时间常数 ~100 次更新
  (~4.5h)，跟不上物理-era 奖励尺度的 3 倍摆动 → 归一化目标/值互相追飞；
  且 value loss 用 MSE，评论家原始输出冲到 ~46σ 时二次放大（实测 loss 1316）。
- **修复**: (a) lpha 0.01 → 0.1（~10 次更新内跟踪尺度）；(b) value loss
  mse → smooth_l1(beta=5)（越界线性回拉，梯度有界但非零）。单测 3 例
  （快/慢 EMA 对照、normalize↔denormalize 往返、零方差优势守卫）。
- **预期**: 谷期不再出现 value 爆炸 → 振荡振幅收窄；本段结束后生效。


### 规则反馈修正: 相对结局判别 (2026-09-26) 🐛

- **事故**: 规则结果反馈（前一条接线）用 ep_ret > 0 判成功 —— 在修复后的
  高回报制度下（mean_ret ~1500）**任何被用规则必然成功** → 置信度虚涨 →
  单一规则 #336805 捕获动作偏置（seg3 匹配 97+ 次）→ op 0.778→0.597→0.297
  崩溃（11.50M→11.60M，100k 步内）。
- **修复**: 判定改为**相对均值**（ep_ret - last_curr_mean_ret，高于平均
  = 成功；与 self-model 的 progress 口径一致）→ 置信度在有优劣区分的
  信号上漂移（~50%），不再单点 runaway。
- **复盘**: 规则闭环首启即暴露此问题（seg3 门控预计失败，链条自动停止）；
  从 11.50M（op 0.778）用修正代码重启。
...
- EWC 修复部署后, chain7 seg1 (10.5M→) 的 op 全程跳升 (逐 event 诊断,
  50k 粒度): 10.55M=0.689/0.698 (二次测量), 10.60M=0.767/0.775 (二次),
  10.65M=0.760, 10.70M=0.644 — **四个连续检查点全部 ≥0.64 (三次 ≥0.69)**;
  复核测量复现 ±0.01。
- **对比**: 此前三条件共 23 个点 (脚本探针 9 / 无探针 4 / 学习探针 10)
  全部落在 **0.12-0.66** 振荡 (均值 0.30-0.35)。
- **解读**: 振荡根因 = **灾难性遗忘**。无 EWC 时每 ~26 分钟的 sleep 周期/
  任务切换持续侵蚀追踪技能再重学 → 双模式振荡; EWC 首次真正生效
  (has_consolidated: True, Fisher L1 3.9→64.9, 失败 0 次) 后技能保持 →
  regime change。"探针门控"仅部分改善 (早段优势、尾段消失) 与"遗忘是
  主因"一致。
- 待确认: multi-seed (7/123) 稳健性 (进行中) + 至 11M 的持续窗口 +
  全量 stage18 eval + ToM 门 (0.55)。

### EWC 修复: consolidate 需在 train 模式跑 Fisher (2026-09-22) ✅
...
- 测试 +2 (模式切换/恢复 + RNN 模型 eval 调用); 10.5M 重启生效, 效果见上。
...
- **验证**: 800k best + 400k 训练 (M2 fix#2 阈值 0.85) → 1.2M eval:
  far 0.37 (0.33-0.41), cross 0.37, ToM 0.28-0.35 — 对比 1.6M (fix#1
  副作用) far 0.20。**退化是 M2 检索 bug 所致, 非训练方法缺陷**。
- **意义**: 第三次"延长退化" (20z 1.3M→2.6M / 21 1.8M→3.2M / s20hyp
  800k→1.6M) 共因 = 记忆注入污染 (弱相关 skill delta 扰动策略)。修复后
  主体可健康长训 — "发育路线主体连续越训越好"的假设重新成立。
- **1.2M ckpt = 新 best** (far/cross 0.37, ToM 0.31, means_ends/physics
  满分)。
- 检索 sim 分布验证: median 0.92 (fix#2 后高相关注入), 无 0.6-0.7 弱
  相关命中。
- **待续**: 继续长训验证主体可持续性 (1.2M→2M+), 或转规则激活实验
  (规则→行为调制, 此前验证极弱)。

### 待办 · 1.6M 续训完成 — 延长退化模式第三次确认 + M2 修复副作用 (2026-09-09)

- **1.6M 最终 eval vs 800k**: far 0.36→0.20, cross 0.36→0.20, ToM 0.31→
  0.15-0.23。第三次"延长训练泛化退化" (21: 1.8M→3.2M 0.44→0.12;
  s20hyp: 800k→1.6M 0.36→0.20)。训练内 tgate 0.73-0.93 保持高。
- **根因候选 (时间吻合)**: 800k→1.6M 唯一重大变化 = M2 retrieve 修复
  (cosine argmax → score 选择)。score 选择引入弱相关旧技能 (sim 分布
  median 0.779, min 0.602; 旧检索全 0.98+) → 不匹配 skill delta 注入
  扰动行为 (类比: 弱相关旧记忆干扰决策)。
- **M2 fix#2**: 检索阈值 0.6→0.85 — 只注入"相关且高价值"技能, 弱相关
  宁可不注入 (策略自主)。train.py L2646。
- **待验证**: 修正阈值后重新训练段, 确认 far 不再退化; 若仍退化则
  退化另有机制 (需诊断: 训练后期行为窄化?)。
...
- **最终读数**: 4000K eval `op=0.10` (p2 链自动完成)。整条 op 曲线:
  3300=0.10 → 3500=0.03 → 3600=0.05 → 3700=0.05 → 3800=0.08 →
  3900=0.06 → 4000=0.10 — **死在 0.03-0.10 区间, 未达 0.6**
- **关键胜利 (稳定性)**: 自 07:49 起 **14 小时零人工干预** —
  watchdog v2 凌晨自动识别 value 爆炸 (v=1048) 回滚重启, 7:49 后
  训练连续健康运行至自然终点; 20h 全程三种崩溃形态 (NaN 级联 /
  logits 爆炸 / value 螺旋) 均被防线+watchdog 消灭或自愈
- **诊断结论 (教师依赖)**: 模仿全程有效 (bc 1.5-1.9, ent 1.6-1.8,
  mean_ret 280-510, 训练奖励涨) 但 **op 不动** — 训练窗口 35% 步由
  teacher 接管, 策略学到"跟随接管", 评测时 teacher=0, 无自主启动
  追踪的能力; 简化版: BC 传授了"找", 没传授"为什么/何时自己找"
- **20i 方向 (记录在案, 未实施)**: ① teacher 退火终值 0.35→0.05
  (自主压力) ② 评测对齐: eval 断言改为"窗口内自主接近" (同一模型
  权重下比较) ③ "安静起始"课程: 窗口头 20 步 teacher 不接管, 让
  策略先自主尝试
- 收尾: 训练/watchdog 已停, ckpt_stage20_004000000.pt 为最终权重
