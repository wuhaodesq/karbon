...
def train(config: dict[str, Any], smoke_only: bool, resume: Path | None) -> int:
    ...
    if stage >= 3 and wm_cfg and bool(wm_cfg.get("enabled", True)):
        ...
        wm = RSSM(RSSMConfig(
            ...
            recon_pixel_weight=float(wm_cfg.get("recon_pixel_weight", 0.0)),
    ...
    if curriculum is not None:
        saved_task_id = 0
        if resume is not None and _resume_extra is not None:
            saved_task_id = int(_resume_extra.get("curriculum_active_task_id", 0))
        for tid in curriculum._tasks:
            if tid == saved_task_id:
