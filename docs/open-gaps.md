...
## 五、外部独立评审风险提示(2026-07-20)

> 一份独立代码/路线评审的结论要点(非项目内部自评),作为可信外部视角存档。
> 整体高度肯定工程纪律、文档诚实度、六大公理贯穿、可验证里程碑阶梯;
> 同时指出以下**最大风险**与待补实证,与本项目 open-gaps 判断一致。

- **最大风险 = 环境承载上限。** 评审明确指出:"唯一最大的风险不是技术方案,
  而是 PhysicsSandbox 环境是否能承载更高级认知能力的评估。" 当前 2D 刚体沙盒
  回报天花板已验证(~102–104),交互类型有限(推/碰)。**这直接支撑 B#7 / C#8
  means_ends 必须靠 3D 世界 + 社会教师 + MiniGrid 指令遵循 + 神经符号支线落地**——
  优先级由此锁定,不是可选项。
- **M2 技能复用缺实证规模。** 代码路径(retrieve→skill_delta 注入)已闭合,但
  `usage_count > 1` 的实际复用频率尚无数。→ 等 Stage 6 跑起、skill_library 有
  数据后,跑一次 `usage_count` 统计补实证(并入"C#8 评测接入退出复验"那条)。
- **train.py 单体过大**(PPO loop + 认知 + imagination + 生长 + 规划 同文件),
  是后续维护痛点。→ 列为已知技术债,**不在当前发育主线动它**(重构有回归风险,
  且铁律#0 不鼓励无收益大改);待 Step 4+ 有稳定 ckpt 再酌情拆分。
- **LLMFusion 语言锚点未验证效果**:已实现但延后到 Step 6 末激活,训练中效果
  待实证。→ 与 ROADMAP:258 延后决策一致,非阻塞。
- **硬件依赖层次高**:本地仅 CPU、无 .venv,真实训练依赖云/家用 GPU。→ 非代码
  问题;远程 Stage 5→6 接力已通过 handoff 脚本自动化。

评审与本项目 open-gaps 判断**高度一致,无新发现的遗漏**;唯一事实修正:评审称
"认知模块只在单 env 激活"已过时——内驱力/情感/因果/数感/规则/核心知识均已在
Stage 6 config 启用(仅受 2D 单 env 架构约束,非未激活)。
